...
class PurchaseInvoiceController extends Controller
{
    ...
    {
        Log::info('Purchase Invoice Store Started', [
            'user_id' => auth()->id(),
            'request' => $request->all()
        ]);
        ...
        try {
            ...
            Log::info('Invoice header created', ['invoice_id' => $invoice->id]);
            ...
            foreach ($request->items as $itemData) {
                ...
                // ✅ FIXED: Only update stock if a variation was actually selected
                if (!empty($itemData['variation_id'])) {
                    ...
                    Log::info('No variation selected, stock update skipped');
                }
            }

            Log::info('Total invoice amount calculated', [
            ...
            Log::info('Inventory account found', [
            ...
            if ($request->hasFile('attachments')) {
                ...
                foreach ($request->file('attachments') as $file) {
                    ...
                    Log::info('Attachment saved', ['path' => $path]);
                }
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            foreach ($invoice->items as $oldItem) {
                if ($oldItem->variation_id) {                     // ← FIX: guard before lookup
            ...
            foreach ($request->items as $itemData) {
                ...
                $variationId = $itemData['variation_id'] ?? null; // ← FIX: use null coalescing
                ...
                $invoice->items()->create([
                    'item_id'      => $itemData['item_id'],   // ← was wrongly 'product_id'
                    'variation_id' => $variationId,
                    'location_id'  => $itemData['location_id'],
                    'quantity'     => $qty,
                    'unit'         => $itemData['unit'] ?? null,  // ← was wrongly 'unit_id'
                    'price'        => $price,
                ]);

                // Increment stock for the new/updated variation
                if ($variationId) {                               // ← FIX: guard the lookup
