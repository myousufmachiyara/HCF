...
class StockTransferController extends Controller
{
    ...
    {
        $query = StockTransfer::with(['fromLocation', 'toLocation', 'creator', 'details.product']);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            Log::info('ST Update Started', [
                'transfer_id'      => $id,
                'old_from'         => $transfer->from_location_id,
                'old_to'           => $transfer->to_location_id,
                'old_details'      => $transfer->details->toArray(),
                'new_from'         => $request->from_location_id,
                'new_to'           => $request->to_location_id,
                'new_items'        => $request->items,
            ]);

            // 1. Reverse using OLD location data
            Log::info('ST Update: Reversing stock movements');
            $this->reverseStockMovements($transfer);

            // 2. Clear old details
            Log::info('ST Update: Deleting old details');
            $transfer->details()->delete();

            // 3. Update parent with NEW location data
            Log::info('ST Update: Updating transfer header');
            $transfer->update($request->only(['date', 'remarks', 'from_location_id', 'to_location_id']));
            $transfer->refresh();

            // 4. Process NEW items
            Log::info('ST Update: Processing new stock items');
            $this->processStockItems($transfer, $request->items);

            DB::commit();
            Log::info('ST Update: Success', ['transfer_id' => $id]);

            return redirect()->route('stock_transfer.index')->with('success', 'Transfer updated.');

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('ST Update Failed', [
                'transfer_id' => $id,
    ...
    private function reverseStockMovements(StockTransfer $transfer)
    {
        ...
        foreach ($transfer->details as $detail) {
            ...
            if ($toLocation && $toLocation->type !== 'customer') {
                $lot = StockLot::where('location_id', $toLocation->id)
                    ->where('product_id', $detail->product_id)
                    ->where('lot_number', $detail->lot_number)
                    ->when($detail->variation_id, fn($q) => $q->where('variation_id', $detail->variation_id))
                    ->first();

                $availableQty = $lot ? $lot->quantity : 0;

                if ($availableQty <= 0) {
                    Log::warning('ST Reversal: Lot fully consumed, skipping', [
                        'lot' => $detail->lot_number, 'product' => $detail->product_id,
                    ]);
                    continue;
                }

                $deductQty = min($detail->quantity, $availableQty);

                if ($deductQty < $detail->quantity) {
                    Log::warning('ST Reversal: Partial deduction', [
                        'lot' => $detail->lot_number, 'expected' => $detail->quantity, 'actual' => $deductQty,
                    ]);
                }

                $this->deductFromLot($toLocation->id, $detail->product_id, $detail->variation_id, $detail->lot_number, $deductQty);
            }
        }
    }
}